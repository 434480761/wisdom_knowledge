...
public class EducationRelationDaoImpl implements EducationRelationDao {
    ...
    private List<RelationForQueryViewModel> queryResDataByRelation(String resType,
        ...
        String commonSelect = "b.identifier AS identifier, b.title AS title, b.description AS description, b.tags AS tags, b.keywords AS keywords,b.custom_properties as customProperties,b.estatus as resourceStatus,rr1.identifier AS relation_id, rr1.relation_type AS relation_type, rr1.label AS label, rr1.order_num AS order_num, rr1.enable AS enable, rr1.tags AS relation_tags,rr1.resource_target_type AS target_type, rr1.creator AS creator,rr1.estatus AS status, rr1.create_time AS create_time, rr1.last_update AS last_update";
        ...
        StringBuffer querySql = commonSql(querySqlTmp, targetType, categories, coverage, label, tags, relationType, reverse, isPortal, isRecycled);
    ...
    private long queryResDataByRelationForQuestion(String resType,
        ...
        // 查数据的sql
        String resourceSql = resourceCommonSqlForQuestion(targetType, categories, coverage, isPortal, isRecycled);
...
//        querySql.append(" LIMIT ").append(start).append(",").append(offset);
    ...
    private long queryResCountByRelation(String resType,
        ...
        StringBuffer countSql = commonSql(countSqlTmp, targetType, categories, coverage, label, tags, relationType, reverse, isPortal, isRecycled);
    ...
    private StringBuffer commonSql(StringBuffer stringBuffer,
        ...
        if (StringUtils.isNotEmpty(relationType)) {
        	...
        	buffer.append(" AND rr.relation_type = 'ASSOCIATE'");
        }
        
        if(isRecycled){
        	buffer.append(" AND (a.enable = 0 AND a.estatus = '" + LifecycleStatus.RECYCLED.getCode() + "')");
        }else{
        	buffer.append(" AND a.enable = 1 ");
        }
        
        buffer.append(" AND rr.enable = 1)");
    ...
    private String relationCommonSqlForQuestion(String resType,
        ...
        if (StringUtils.isNotEmpty(relationType)) {
        	...
        	querySqlTmp.append(" AND rr.relation_type = 'ASSOCIATE'");
        }
    ...
    private String resourceCommonSqlForQuestion(String targetType,
        ...
        String commonSelect = "SELECT a.identifier AS identifier, a.title AS title, a.description AS description, a.tags AS tags, a.keywords AS keywords,a.custom_properties as customProperties,a.estatus as resourceStatus ";
        ...
        querySqlTmp.append("a.primary_category = :targetType AND a.identifier IN (:sourceUuids) ");
        if(isRecycled){
        	querySqlTmp.append(" AND (a.enable = 0 AND a.estatus = '" + LifecycleStatus.RECYCLED.getCode() + "') ");
        }else{
        	querySqlTmp.append(" AND a.enable = 1 ");
        }
        
        querySqlTmp.append(" ORDER BY INSTR (:sourceUuidStr, a.identifier)");
        return querySqlTmp.toString();
    }
    ...
    private void setData(List<ResultModel> resultList,
        ...
        for(ResultModel resultModel : resultList) {
            ...
            if(StringUtils.isNotEmpty(resultModel.getCustomProperties())){
            	viewModel.setCustomProperties(ObjectUtils.fromJson(resultModel.getCustomProperties(), Map.class));
            }
