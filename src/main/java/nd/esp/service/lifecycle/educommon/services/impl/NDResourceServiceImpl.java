...
public class NDResourceServiceImpl implements NDResourceService {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(NDResourceServiceImpl.class);
	...
	static {
		...
		Condition skipIds = new Condition() {
			...
			public boolean applies(MappingContext context) {
				String name = context.getMapping().getLastDestinationProperty()
						.getName();
				boolean flag = true;
	...
	private static List<String> TRANSCODE_TYPES = Arrays.asList(new String[] {
			"coursewares", "assets", "lessonplans", "learningplans",
			"teachingmaterials" });

	@Override
	public ListViewModel<ResourceModel> resourceQueryByEla(String resType,
			List<String> includes, Set<String> categories,
			Set<String> categoryExclude, List<Map<String, String>> relations,
			List<String> coverages, Map<String, Set<String>> propsMap,
			Map<String, String> orderMap, String words, String limit,
			boolean isNotManagement, boolean reverse, Boolean printable,
			String printableKey) {
		// 返回的结果集
		...
		Map<String, Map<String, List<String>>> params = this
				.dealFieldAndValues(categories, categoryExclude, relations,
						coverages, propsMap, isNotManagement, printable,
						printableKey);
		Integer result[] = ParamCheckUtil.checkLimit(limit);
		if (includes == null) {
			includes = new ArrayList<String>();
		}
		listViewModel = eS_Search.searchByES(resType, includes, params,
				orderMap, result[0], result[1]);
	 ...
	 */
	@Override
	public ListViewModel<ResourceModel> resourceQueryByTitanWithStatistics(
			Set<String> resTypeSet, List<String> includes,
			...
			String words, String limit, boolean isNotManagement,
			boolean reverse, Boolean printable, String printableKey,
		...
		Map<String, Map<String, List<String>>> params = this
				.dealFieldAndValues(categories, categoryExclude, relations,
						coverages, propsMap, isNotManagement, printable,
						printableKey, forceStatus);
		// FIXME 处理orderMap 暂时放在这里
		Map<String, String> orders = TitanUtils.dealOrderMap(orderMap,
				showVersion, reverse, relations, statisticsType,
				statisticsPlatform);
		...
		listViewModel = titanSearchService.searchWithStatistics(resTypeSet,
				includes, params, orders, result[0], result[1], reverse, words,
	 ...
	 */
	@Override
	public ListViewModel<ResourceModel> resourceQueryByTitanES(
			Set<String> resTypeSet, List<String> fields, List<String> includes,
			Set<String> categories, Set<String> categoryExclude,
			List<Map<String, String>> relations, List<String> coverages,
			Map<String, Set<String>> propsMap, Map<String, String> orderMap,
			String words, String limit, boolean isNotManagement,
		...
		listViewModel = titanSearchService.searchUseES(resTypeSet, fields,
				includes, params, orderMap, result[0], result[1], reverse,
				words);
		if (listViewModel != null)
			listViewModel.setLimit(limit);
		return listViewModel;
	...
	private Map<String, Map<String, List<String>>> dealFieldAndValues(
			Set<String> categories, Set<String> categoryExclude,
			List<Map<String, String>> relations, List<String> coverages,
			Map<String, Set<String>> propsMap, boolean isNotManagement,
			Boolean printable, String printableKey) {
		return this.dealFieldAndValues(categories, categoryExclude, relations,
				coverages, propsMap, isNotManagement, printable, printableKey,
				false);
	}
	...
	private List<String> coverageParam4DealOnline(List<String> coverages,
		...
		for (String coverage : coverages) {
			...
			if (coverageElemnt.get(0).equals(
					CoverageConstant.TargetType.TARGET_TYPE_ORG.getCode())
					&& coverageElemnt.get(1).equals(
							CoverageConstant.ORG_CODE_ND)) {// ND库
				if (!forceStatus) {
	...
	@Override
	public ListViewModel<ResourceModel> resourceQueryByDB(String resType,
			String resCodes, List<String> includes, Set<String> categories,
			Set<String> categoryExclude, List<Map<String, String>> relations,
			List<Map<String, String>> relationsExclude, List<String> coverages,
			Map<String, Set<String>> propsMap, Map<String, String> orderMap,
			String words, String limit, boolean isNotManagement,
			boolean reverse, Boolean printable, String printableKey,
			boolean firstKnLevel, String statisticsType,
			String statisticsPlatform, boolean forceStatus, List<String> tags,
			boolean showVersion, boolean isEnable) {

		ListViewModel<ResourceModel> rListViewModel = new ListViewModel<ResourceModel>();
		rListViewModel.setLimit(limit);

		// 判断使用IN还是EXISTS
		boolean useIn = ndResourceDao.judgeUseInOrExists(resType, resCodes,
				categories, categoryExclude, relations, coverages, propsMap,
				words, isNotManagement, reverse, printable, printableKey,
				firstKnLevel, forceStatus, tags, showVersion, isEnable);

		// 查总数和Items使用线程同时查询
		List<Callable<QueryThread>> threads = new ArrayList<Callable<QueryThread>>();
		QueryThread countThread = new QueryThread(true, resType, resCodes,
				includes, categories, categoryExclude, relations,
				relationsExclude, coverages, propsMap, null, words, limit,
				isNotManagement, reverse, useIn, printable, printableKey,
				firstKnLevel, statisticsType, statisticsPlatform, forceStatus,
				tags, showVersion, isEnable);
		QueryThread queryThread = null;
		if (ndResourceDao.judgeUseRedisOrNot("(0,1)", isNotManagement,
				coverages, orderMap)) {// 如果是走Redis的,useIn=true
			queryThread = new QueryThread(false, resType, resCodes, includes,
					categories, categoryExclude, relations, relationsExclude,
					coverages, propsMap, orderMap, words, limit,
					isNotManagement, reverse, true, printable, printableKey,
					firstKnLevel, statisticsType, statisticsPlatform,
					forceStatus, tags, showVersion, isEnable);
		} else {
			queryThread = new QueryThread(false, resType, resCodes, includes,
					categories, categoryExclude, relations, relationsExclude,
					coverages, propsMap, orderMap, words, limit,
		...
		try {
			List<Future<QueryThread>> results = executorService.invokeAll(
					threads, 10 * 60, TimeUnit.SECONDS);
			for (Future<QueryThread> result : results) {
				try {
					...
					throw new LifeCircleException(
							...
							"LC/COMMON_QUERY_GET_FAIL", e.getMessage());
				}
			}
		} catch (InterruptedException e) {
			throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
					"LC/QUERY_THREAD_FAIL", e.getMessage());
		}
	...
	class QueryThread implements Callable<QueryThread> {
		...
		QueryThread(boolean isCount, String resType, String resCodes,
				List<String> includes, Set<String> categories,
				Set<String> categoryExclude,
				List<Map<String, String>> relations,
				List<Map<String, String>> relationsExclude,
				List<String> coverages, Map<String, Set<String>> propsMap,
				Map<String, String> orderMap, String words, String limit,
				boolean isNotManagement, boolean reverse, boolean useIn,
		...
		public QueryThread call() throws Exception {
			if (isCount) {
				this.total = ndResourceDao.commomQueryCount(resType, resCodes,
						categories, categoryExclude, relations,
						relationsExclude, coverages, propsMap, words, limit,
						isNotManagement, reverse, useIn, printable,
						printableKey, firstKnLevel, forceStatus, tags,
						showVersion, isEnable);
			} else {
				this.items = ndResourceDao.commomQueryByDB(resType, resCodes,
						includes, categories, categoryExclude, relations,
						relationsExclude, coverages, propsMap, orderMap, words,
						limit, isNotManagement, reverse, useIn, printable,
						printableKey, firstKnLevel, statisticsType,
	...
	@Override
	public ListViewModel<ResourceViewModel> resourceQuery4IntelliKnowledge(
			List<String> includesList, String chapterId, String pageSize,
			String offset) {
		...
		sb.append(LifeCircleApplicationInitializer.properties
				.getProperty("intelli.uri"));
		...
		try {
			...
			if ("SMARTQ/NOT_FOUND".equalsIgnoreCase(e.getRemoteResponseEntity()
					.getBody().getCode())) {
		...
		if (CollectionUtils.isNotEmpty(response)) {
			if (CollectionUtils.isNotEmpty((List<Map>) (response.get("rows")))) {
				...
				for (Map map : (List<Map>) (response.get("rows"))) {
					ResourceViewModel rvm = ObjectUtils.fromJson(
							ObjectUtils.toJson(map), ResourceViewModel.class);
					...
					if (!haveEDU) {
						...
						rem.setSemanticDensity(((Map) map.get("education_info"))
								.get("semantic_density") == null ? 0L : Long
								.parseLong((String) ((Map) map
										.get("education_info"))
										.get("semantic_density")));
						rem.setDifficulty(((Map) map.get("education_info"))
								.get("difficulty") == null ? null
								: (String) ((Map) map.get("education_info"))
										.get("difficulty"));
						rem.setLearningTime(((Map) map.get("education_info"))
								.get("learning_time") == null ? null
								: (String) ((Map) map.get("education_info"))
										.get("learning_time"));
						rem.setDescription(((Map) map.get("education_info"))
								.get("edu_description") == null ? null
								: (Map) ((Map) map.get("education_info"))
										.get("edu_description"));
						rem.setLanguage(((Map) map.get("education_info"))
								.get("edu_language") == null ? null
								: (String) ((Map) map.get("education_info"))
										.get("edu_language"));
					...
					if (!haveTI) {
						...
						rti.setLocation((String) ((Map) ((Map) map
								.get("tech_info")).get("href")).get("location"));
				...
				rListViewModel.setTotal(((Integer) response.get("total"))
						.longValue());
			} else {
				rListViewModel.setTotal(0L);
				rListViewModel.setItems(new ArrayList<ResourceViewModel>());
	...
	public Map<String, Integer> resourceStatistics(String resType,
		...
		return ndResourceDao.resourceStatistics(resType, categories, coverages,
				propsMap, groupBy, isNotManagement, firstKnLevel);
	}
	 ...
	 * 
	 * @see
	 * nd.esp.service.lifecycle.educommon.services.LsmService#getDetail(java
	 * .lang.String)
	 */
	@Override
	public ResourceModel getDetail(String resourceType, String uuid,
			List<String> includeList) {
		return getDetail(resourceType, uuid, includeList, false);
	}

	private ResourceModel changeToModel(EspEntity bean, String resourceType,
			List<String> includeList) {
		if (bean == null) {
			return null;
		}
		// 忽略categoryList, techInfoList
		ResourceModel model = specialModelMapper.map(bean,
				commonServiceHelper.getModel(resourceType));
		try {
			...
			if (f != null) {
				...
				Method m = model.getClass().getMethod("setExtProperties",
						f.getType());
				m.invoke(model, o);
			}
		} catch (Exception e) {
	...
	private void addIncludes(List<String> includes,
			ResourceModel resourceModel, EspEntity ndResource, String resType)
			throws EspStoreException {
		for (String include : includes) {
			if (include.equals(IncludesConstant.INCLUDE_TI)) {
				...
				List<TechInfo> techInfos = commonServiceHelper
						.getTechInfoRepositoryByResType(resType)
						.getAllByExample(techInfo);
				...
				ResEducationalModel resEducationalModel = specialModelMapper
						.map(ndResource, ResEducationalModel.class);
				// description和language属性名不一致,单独处理
				if (ndResource instanceof Education) {
					resEducationalModel.setDescription(((Education) ndResource)
							.getEduDescription());
					resEducationalModel.setLanguage(((Education) ndResource)
							.getEduLanguage());
				}
				resourceModel.setEducationInfo(resEducationalModel);
			} else if (include.equals(IncludesConstant.INCLUDE_LC)) {
				ResLifeCycleModel resLifeCycleModel = specialModelMapper.map(
						ndResource, ResLifeCycleModel.class);
				resourceModel.setLifeCycle(resLifeCycleModel);
			} else if (include.equals(IncludesConstant.INCLUDE_CR)) {
				ResRightModel resRightModel = specialModelMapper.map(
						ndResource, ResRightModel.class);
				// description/right属性名不一致,单独处理
				if (ndResource instanceof Education) {
					resRightModel.setDescription(((Education) ndResource)
							.getCrDescription());
					resRightModel.setRight(((Education) ndResource)
							.getCrRight());
				}
				...
				List<ResourceCategory> resourceCategories = commonServiceHelper
						.getResourceCategoryRepositoryByResType(resType)
						.getAllByExample(resourceCategory);

				if (!CollectionUtils.isEmpty(resourceCategories)) {
					for (ResourceCategory rc : resourceCategories) {
						resClassificationModels
								.add(changeResourceCategoryToModel(rc));
					}
				}
	 ...
	 * 
	 * @see
	 * nd.esp.service.lifecycle.educommon.services.NDResourceService#batchDetail
	 * (java.lang.String, java.util.Set, java.util.List)
	 */
	@Override
	public List<ResourceModel> batchDetail(String resourceType,
			Set<String> uuidSet, List<String> includeList) {
	...
	@Override
	public List<ResourceModel> batchDetail(String resourceType,
			Set<String> uuidSet, List<String> includeList, Boolean isAll) {
		return batchDetail(resourceType, uuidSet, includeList, isAll, false);
	}
	...
	public List<ResourceModel> batchDetail(String resourceType,
		...
		try {
			beanListResult = resourceRepository.getAll(new ArrayList<String>(
					uuidSet));
		} catch (EspStoreException e) {
		...
		//
		// LOG.error(LifeCircleErrorMessageMapper.ResourceNotFound.getMessage()
		...
		List<String> newIncludeList = new ArrayList<String>(includeList);// 需要拷贝一份，controller
																			// 也要使用到includeList
		...
		if (isRecycled) {
			for (EspEntity espEntity : beanListResult) {
				if (espEntity != null
						...
						&& ((Education) espEntity).getStatus().equals(
								LifecycleStatus.RECYCLED.getCode())
	 ...
	 * @since
	 */
	private void addCG(List<ResourceModel> resourceModels, String resourceType,
			Set<String> uuids) {
		if (CollectionUtils.isEmpty(uuids)
				|| CollectionUtils.isEmpty(resourceModels)) {
			return;
		}
		List<String> resourceTypes = new ArrayList<String>();
		resourceTypes.add(resourceType);
		List<ResourceCategory> resourceCategories = ndResourceDao
		...
		if (CollectionUtils.isNotEmpty(resourceCategories)) {
			for (ResourceCategory resourceCategory : resourceCategories) {
				if (resourceCategory != null) {
					...
					List<ResClassificationModel> resClassificationModels = resClassificationModelMap
							.get(key);
					if (resClassificationModels == null) {
						resClassificationModels = new ArrayList<ResClassificationModel>();
						resClassificationModelMap.put(key,
								resClassificationModels);
					}
					resClassificationModels
							.add(changeResourceCategoryToModel(resourceCategory));
				}
			}
		}
		...
		if (CollectionUtils.isNotEmpty(resourceModels)) {
			for (ResourceModel resourceModel : resourceModels) {
				if (resourceModel != null) {
					resourceModel.setCategoryList(resClassificationModelMap
	 ...
	 * @since
	 */
	private ResClassificationModel changeResourceCategoryToModel(
			ResourceCategory resourceCategory) {
		return BeanMapperUtils.beanMapper(resourceCategory,
	 ...
	 * @since
	 */
	private void addTI(List<ResourceModel> resourceModels, String resourceType,
			Set<String> uuids) {
		if (CollectionUtils.isEmpty(uuids)
				|| CollectionUtils.isEmpty(resourceModels)) {
			return;
		...
		List<String> resourceTypes = new ArrayList<String>();
		resourceTypes.add(resourceType);
		List<TechInfo> techInfos = ndResourceDao.queryTechInfosUseHql(
				resourceTypes, uuids);
		...
		if (CollectionUtils.isNotEmpty(techInfos)) {
			for (TechInfo techInfo : techInfos) {
				if (techInfo != null) {
					...
					List<ResTechInfoModel> resTechInfoModels = resTechInfoModelMap
							.get(key);
		...
		if (CollectionUtils.isNotEmpty(resourceModels)) {
			for (ResourceModel resourceModel : resourceModels) {
				if (resourceModel != null) {
					resourceModel.setTechInfoList(resTechInfoModelMap
							.get(resourceModel.getIdentifier()));
				}
			}
		}
	...
	private ResTechInfoModel changeTechInfoToModel(TechInfo techInfo) {
		ResTechInfoModel rtim = BeanMapperUtils.beanMapper(techInfo,
				ResTechInfoModel.class);
		// requirements单独处理
		List<TechnologyRequirementModel> req = ObjectUtils.fromJson(
				techInfo.getRequirements(),
	...
	public void delete(String resourceType, String uuid) {
		...
		ResourceRepository resourceRepository = commonServiceHelper
				.getRepository(resourceType);
		...
		// 2、判断该资源下有没有在用的版本
		List<Map<String, Object>> list = ndResourceDao.queryResourceByMid(
				resourceType, uuid);
		if (list != null && list.size() > 1) {
			throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
					"LC/DELETE_RESOURCE_DENY", "resourceType:" + resourceType
							+ "uuid:" + uuid + "资源存在子版本，无法删除");
		}

		// 所有通用接口支持的资源都继承了NdResource
		// 存在问题，教学目标等，并没有继承NdResource
		((Education) beanResult).setEnable(false);
		((Education) beanResult).setLastUpdate(new Timestamp(new Date()
				.getTime()));
	...
	public void batchDelete(String resourceType, List<String> uuids) {
		...
		ResourceRepository resourceRepository = commonServiceHelper
				.getRepository(resourceType);
		List<EspEntity> beanResults = new ArrayList<>();
		try {
			beanResults = resourceRepository.getAll(uuids);
		} catch (EspStoreException e) {
		...
		if (beanResults.isEmpty()) {
			throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
					LifeCircleErrorMessageMapper.ResourceNotFound.getCode(),
					LifeCircleErrorMessageMapper.ResourceNotFound.getMessage()
							+ "resourceType:" + resourceType + "all not found");
		}
		// 查出来的id
		List<String> foundIds = new ArrayList<>();
		for (EspEntity beanResult : beanResults) {
			String uuid = beanResult.getIdentifier();
			foundIds.add(uuid);
		}
		// id-实体 map
		Map<String, EspEntity> beanResultMap = new HashMap<String, EspEntity>();
		for (EspEntity entity : beanResults) {
			beanResultMap.put(entity.getIdentifier(), entity);
		}
		// 找出没查到的id
		List<String> notFoundIds = new ArrayList<>();
		for (String id : uuids) {
			if (!foundIds.contains(id)
					|| ((Education) beanResultMap.get(id)).getEnable() == null
					|| !((Education) beanResultMap.get(id)).getEnable()
					|| !resourceType.equals(((Education) beanResultMap.get(id))
							.getPrimaryCategory())) {
		...
		if (!notFoundIds.isEmpty()) {
			...
			for(String id:notFoundIds){
				exceptionId.append("{");
				exceptionId.append(id);
				exceptionId.append("}");
			}
			throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
					LifeCircleErrorMessageMapper.ResourceNotFound.getCode(),
					LifeCircleErrorMessageMapper.ResourceNotFound.getMessage()
							+ "resourceType:" + resourceType + "uuid:" + exceptionId.toString());
		}
		// 2、判断资源下有没有在用的版本
		List<Map<String, Object>> list = ndResourceDao.queryResourceByMidList(
				resourceType, uuids);
		// 找出有在用版本的资源id
		if (list != null && list.size() > uuids.size()) {
			String mutipleId = dealExceptionId(list);
			throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
					"LC/DELETE_RESOURCE_DENY", "resourceType:" + resourceType
							+ "uuid:" + mutipleId + "资源存在子版本，无法删除");
		}
		...
		for (EspEntity beanResult : beanResults) {
			...
			LOG.debug("删除类型:{},id:{}资源", resourceType,
					beanResult.getIdentifier());
	 ...
	 */
	@Override
	public AccessModel getUploadUrl(String resourceType, String uuid,
			String uid, Boolean renew, String coverage) {
		String rootPath = "";
		...
		if (uuid.equals(Constant.DEFAULT_UPLOAD_URL_ID)) {
			...
			if (isPersonal(coverage)) {
				// 个人
				rootPath = Constant.CS_INSTANCE_MAP.get(
			...
			if (!renew) {
				...
				ResourceModel resourceModel = getDetail(resourceType, uuid,
						Arrays.asList(IncludesConstant.INCLUDE_TI));

				if (resourceModel == null) {
					// 不存在对应的资源
					LOG.error(LifeCircleErrorMessageMapper.CSResourceNotFound
							.getMessage());
		...
		// String sessionPath = csInstance.getPath().replace("/esp", "");
		// //暂时去除esp
		String sessionPath = StringUtils
				.removeEnd(csInstance.getPath(), "/esp");
	...
	public static String getRootPathFromLocation(String location) {
		...
		if (pathChunk.length < ND_AND_PERSON_ROOT_PATH_LENGTH + 1) {
			...
			LOG.error(LifeCircleErrorMessageMapper.CSFilePathError.getMessage()
					+ location);

			throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
					LifeCircleErrorMessageMapper.CSFilePathError.getCode(),
					LifeCircleErrorMessageMapper.CSFilePathError.getMessage()
							+ location);
		}
		StringBuilder builder = new StringBuilder();
		if (ND_AND_PERSON_DEFAUL_ORG.equals(pathChunk[2])) {
			// 个人或者是nd
			builder.append("/").append(pathChunk[1]).append("/")
					.append(pathChunk[2]);
		} else {
			// 其它组织
			if (pathChunk.length < OTHER_ORG_ROOT_PATH_LENGTH + 1) {
				...
				LOG.error(LifeCircleErrorMessageMapper.CSFilePathError
						.getMessage() + location);

				throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
						LifeCircleErrorMessageMapper.CSFilePathError.getCode(),
						LifeCircleErrorMessageMapper.CSFilePathError
								.getMessage() + location);
			}
			builder.append("/").append(pathChunk[1]).append("/")
					.append(pathChunk[2]).append("/").append(pathChunk[3]);
		}
	...
	public static CSInstanceInfo getCsInstanceAccordingRootPath(String rootPath) {
		...
		if (Constant.CS_DEFAULT_INSTANCE.contains(serviceName)) {
			...
			csInfo = Constant.CS_INSTANCE_MAP
					.get(Constant.CS_DEFAULT_INSTANCE_OTHER);
		}

		if (csInfo == null) {

			LOG.error(LifeCircleErrorMessageMapper.CSInstanceKeyNotFound
					.getMessage());
	...
	@Override
	public AccessModel getUploadUrl(String resourceType, String uuid,
			String uid, Boolean renew) {
		return getUploadUrl(resourceType, uuid, uid, renew, "");
	}

	@Override
	public AccessModel getDownloadUrl(String resourceType, String uuid,
		...
		return getDownloadAccessModel(resourceModel, resourceType, uuid, uid,
				key);
	}
	...
	public Map<String, AccessModel> batchGetDownloadUrl(String resourceType,
		...
		// 批量获取资源信息
		List<ResourceModel> resourceModels = batchDetail(resourceType, ids,
				Arrays.asList(IncludesConstant.INCLUDE_TI));
		...
		if (CollectionUtils.isNotEmpty(resourceModels)) {
			for (ResourceModel resourceModel : resourceModels) {
				AccessModel accessModel = new AccessModel();
				try {
					accessModel = getDownloadAccessModel(resourceModel,
							resourceType, resourceModel.getIdentifier(), uid,
							key);
				} catch (Exception e) {
	 ...
	 * @return
	 */
	private AccessModel getDownloadAccessModel(ResourceModel resourceModel,
			String resourceType, String uuid, String uid, String key) {
		CSInstanceInfo csInstanceInfo = null;

		// 获取到目录
		String location = "";
		boolean isPpt2Html = PPT_LOCATION_KEY.equals(key)
				&& IndexSourceType.SourceCourseWareType.getName().equals(
						resourceType);
		...
		// String path = producePath(rootPath, resourceType, uuid); //FIXME
		// 到时需要再综合考虑特殊处理ppt2html

		CSGetSessionParam param = new CSGetSessionParam();
		// param.setPath(path); FIXME 需要特殊处理一下路径
		// param.setPath(csInstanceInfo.getPath());// FIXME 应文鑫要求扩大授权范围
		// 见上传
		String sessionPath = StringUtils.removeEnd(csInstanceInfo.getPath(),
				"/esp");
		LOG.info("sessionPath:{}", sessionPath);
		param.setPath(sessionPath);
		...
		// 获得session
		String sessionid = getSessionIdFromCS(csInstanceInfo.getUrl()
				+ "/sessions", param);
	...
	private String producePptLocation(String root, String uuid) {
		...
		stringBuilder.append(root).append("/assets/ppts/").append(uuid)
				.append(".pkg");
	...
	public Map<String, Object> getResPreviewUrls(String resType, String uuid) {
		...
		ResourceModel resourceModel = getDetail(resType, uuid,
				Arrays.asList(IncludesConstant.INCLUDE_TI));
		...
		if (CollectionUtils.isNotEmpty(techInfoList)) {
			for (ResTechInfoModel resTechInfoModel : techInfoList) {
				...
				if (StringUtils.isNotEmpty(title)
						&& resTechInfoModel.getTitle().equals("source")) {
					sourceFlag = true;
					break;
				}
			}
	...
	@Override
	public Map<String, Object> getResPreviewByHref(String resType,
			String location) {
		Map<String, Object> returnMap = new HashMap<String, Object>();
		String rootPath = getRootPathFromLocation(location);
		...
		if (IndexSourceType.LessonPlansType.getName().equals(resType)
			...
			xmlPath = location.substring(location.indexOf("/")).replace(
					"main.xml", "pages/imagePage.xml");
		} else {
			xmlPath = location.substring(location.indexOf("/"));
		}
		sb.append(xmlPath);
		String responseXml = wafSecurityHttpClient.getForObject(sb.toString(),
				String.class);
		...
		try {
			...
			if (doc != null) {
				...
				if (IndexSourceType.LessonPlansType.getName().equals(resType)
						|| IndexSourceType.LearningPlansType.getName().equals(
								resType)) {
					Element modulesEle = root.getChild("modules");
					if (modulesEle != null) {
						for (Element module : (List<Element>) modulesEle
								.getChildren("addonModule")) {
							if (module.getAttributeValue("addonId").equals(
									"EBookContent")) {
								Element propEle = module.getChild("properties");
								if (propEle != null) {
									for (Element prop : (List<Element>) propEle
											.getChildren("property")) {
										if ("images".equals(prop
												.getAttributeValue("name"))) {
											Element itemsEle = prop
													.getChild("items");
											if (itemsEle != null) {
												for (Element item : (List<Element>) itemsEle
														.getChildren("item")) {
													for (Element itemProp : (List<Element>) item
															.getChildren("property")) {
														if ("src"
																.equals(itemProp
																		.getAttributeValue("name"))) {
	...
	private String getSessionIdFromCS(String url, CSGetSessionParam param) {
		...
		String sessionId = "";
		try {
			Map<String, String> session = wafSecurityHttpClient.post(url,
					requestBody, Map.class);
			sessionId = session.get("session");
		} catch (Exception e) {
			LOG.error(LifeCircleErrorMessageMapper.InvokingCSFail.getMessage(),
					e);
	 ...
	 * @since
	 */
	public static String producePath(String root, String resourceType,
			String uuid) {
		StringBuilder stringBuilder = new StringBuilder(root);
		stringBuilder.append("/").append(resourceType).append("/").append(uuid)
				.append(".pkg");
		return stringBuilder.toString();
	}
	...
	private String assertHasAuthorizationAndGetPath(String coverage, String uid) {
		...
		if (coverageChunk == null || coverageChunk.length < 2) {
			...
			LOG.error(LifeCircleErrorMessageMapper.CSUploadCoverageError
					.getMessage());
		...
		if (repoInfo == null) {
			...
			repoInfo.setRepositoryPath(Constant.CS_SESSION_PATH + "/"
					+ coverage);
	...
	private String getLocation(ResourceModel resourceModel,
		...
		if (CollectionUtils.isNotEmpty(techInfoModels)) {
			for (ResTechInfoModel model : techInfoModels) {
				if (model != null
						&& defaultLocationKey.equals(model.getTitle())) {
		...
		if (StringUtils.isEmpty(location)) {
			...
			LOG.error(LifeCircleErrorMessageMapper.CSFilePathError.getMessage()
					+ "不存在对应于key = " + defaultLocationKey + "的地址");
	 ...
	 * @author linsm
	 * @see Copyright(c) 2009-2014, TQ Digital Entertainment, All Rights
	 *      Reserved
	 */
	 ...
	 * 
	 * @see
	 * nd.esp.service.lifecycle.educommon.services.NDResourceService#getDetail
	 * (java.lang.String, java.lang.String, java.util.List, java.lang.Boolean)
	 */
	@Override
	public ResourceModel getDetail(String resourceType, String uuid,
			List<String> includeList, Boolean isAll) {
		return getDetail(resourceType, uuid, includeList, isAll, false);
	}

	@Override
	public ResourceModel getDetail(String resourceType, String uuid,
			List<String> includeList, Boolean isAll, Boolean isRecycled) {

		EspEntity beanResult = null;
		if (isRecycled) {
			beanResult = checkResourceInRecycled(resourceType, uuid);
		} else {
			beanResult = checkResourceExist(resourceType, uuid, isAll);
		}
	 ...
	 * @return
	 */
	private EspEntity checkResourceInRecycled(String resourceType, String uuid) {
		ResourceRepository<? extends EspEntity> resourceRepository = commonServiceHelper
				.getRepository(resourceType);
		EspEntity beanResult = null;
		try {
		...
		if (beanResult == null
				|| ((Education) beanResult).getEnable() == null
				|| ((Education) beanResult).getEnable()
				|| (!((Education) beanResult).getEnable() && !((Education) beanResult)
						.getStatus().equals(LifecycleStatus.RECYCLED.getCode()))
				|| !resourceType.equals(((Education) beanResult)
						.getPrimaryCategory())) {
			...
			throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
					LifeCircleErrorMessageMapper.ResourceNotFound.getCode(),
					LifeCircleErrorMessageMapper.ResourceNotFound.getMessage()
							+ "回收站中:" + " resourceType:" + resourceType
							+ " uuid:" + uuid);
		}
	 ...
	 * 
	 * @see
	 * nd.esp.service.lifecycle.educommon.services.NDResourceService#create(
	 * java.lang.String,
	...
	public ResourceModel create(String resourceType,
			ResourceModel resourceModel, DbName dbName) {
		// 1、判断主键id是否已经存在
		boolean flag = isDuplicateId(resourceType,
				resourceModel.getIdentifier());
		if (flag) {
			throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
					LifeCircleErrorMessageMapper.CheckDuplicateIdFail);
		}

		// 2、判断资源编码是否唯一
		if (StringUtils.isNotEmpty(resourceModel.getNdresCode())) {
			boolean flagCode = isDuplicateCode(resourceType,
					resourceModel.getIdentifier(), resourceModel.getNdresCode());
			if (flagCode) {
				throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
						LifeCircleErrorMessageMapper.CheckDuplicateCodeFail);
			}
		}
		...
		// 8、同步推送至报表系统
		nds.notifyReport4Resource(resourceType, resourceModel,
				OperationType.CREATE);

		// 9.异步自动创建教学目标（智慧知识需求）
		asynCreateInstructionalObjective(resourceType, resourceModel);
	 ...
	 * @param resourceModel
	 */
	private void asynCreateInstructionalObjective(String resourceType,
			final ResourceModel resourceModel) {
		boolean flag = false;
		if (IndexSourceType.AssetType.getName().equals(resourceType)) {
			List<ResClassificationModel> rcList = resourceModel
					.getCategoryList();
			if (CollectionUtils.isNotEmpty(rcList)) {
				for (ResClassificationModel resClassificationModel : rcList) {
					if (resClassificationModel.getTaxoncode().equals("$RA0503")) {
						flag = true;
						break;
					}
				}
		...
		if (flag) {
			executorService.execute(new Runnable() {
				@Override
				public void run() {
					commonServiceHelper.autoCreateObjectives(resourceModel);
				}
			});
		}
	}
	...
	private boolean isDuplicateId(String resourceType, String identifier) {
		...
		if (uploadable) {
			...
			if (!CommonServiceHelper.isQuestionDb(resourceType)) {
				...
				num = ndResourceDao.queryCountByResId4QuestionDb(resourceType,
						identifier);
			}
			if (num > 0) {
				return true;
	 ...
	 * @return
	 */
	private boolean isDuplicateCode(String resourceType, String identifier,
			String code) {
		int num;
		if (!CommonServiceHelper.isQuestionDb(resourceType)) {
			num = ndResourceDao.queryCodeCountByResId(resourceType, identifier,
					code);
		} else {
			num = ndResourceDao.queryCodeCountByResId4QuestionDb(resourceType,
					identifier, code);
		}
		if (num > 0) {
			return true;
	...
	private boolean dealBasicInfo(String resourceType,
			ResourceModel resourceModel, OperationType operationType,
			Education oldBean) {
		...
		// 所有通用接口支持的资源在sdk 都继承了Education
																				// set
		Education education = changeModelToBean(resourceType, resourceModel); // have
																				// primary_category
		try {
			if (operationType == OperationType.CREATE) {
				...
				if (ResourceNdCode.knowledges.toString().equals(resourceType)) {
					...
					if (StringUtils.isNotBlank(knowledgeModel
							.getExtProperties().getRootNode())) {
						knowledge.setTeachingMaterial(knowledgeModel
								.getExtProperties().getRootNode());
					} else {
						knowledge
								.setTeachingMaterial(getSubjectWithCheck(resourceModel));
					}
					TreeDirection treeDirection = TreeDirection
							.fromString(knowledgeModel.getExtProperties()
									.getDirection());
					TreeModel current = treeService.insertLeaf(
							getTreeTargetAndParent(knowledgeModel, knowledge),
							treeDirection);
				...
				LOG.debug("创建资源类型:{},uuid:{}", resourceType,
						education.getIdentifier());
				...
				if (ResourceNdCode.knowledges.toString().equals(resourceType)) {
					...
					knowledge.setTeachingMaterial(oldKnowledge
							.getTeachingMaterial());
				...
				// customProperties存放各业务系统的自定义数据，避免被其它业务系统误改 add by xuzy
				// 20160921
				if (StringUtils.isEmpty(education.getCustomProperties())) {
					education
							.setCustomProperties(oldBean.getCustomProperties());
				}
				...
				education.setLastUpdate(new Timestamp(System
						.currentTimeMillis()));

				LOG.debug("调用sdk方法：update");
				LOG.debug("修改资源类型:{},uuid:{}", resourceType,
						education.getIdentifier());
	 ...
	 */
	@Override
	public TreeTrargetAndParentModel getTreeTargetAndParent(
			KnowledgeModel knowledgeModel, Chapter knowledge) {
		TreeTrargetAndParentModel model = new TreeTrargetAndParentModel();
		// target and parent
		String mid = knowledge.getTeachingMaterial();

		if (StringUtils.isNotEmpty(knowledgeModel.getExtProperties()
				.getTarget())) {
			Chapter targetChapter = getKnowledgeFromSdkWithCheck(
					knowledge.getPrimaryCategory(), mid, knowledgeModel
							.getExtProperties().getTarget());
			model.setTarget(new TreeModel(targetChapter));
		} else if (StringUtils.isNotEmpty(knowledgeModel.getExtProperties()
				.getParent())) {
			if (!knowledgeModel.getExtProperties().getParent().equals("ROOT")) {
				Chapter parentKnowledge = getKnowledgeFromSdkWithCheck(
						knowledge.getPrimaryCategory(), mid, knowledgeModel
								.getExtProperties().getParent());
				model.setParent(new TreeModel(parentKnowledge));
			} else {
				// root 默认为mid

				int nodeNum = (int) chapterDao.countQueryChapterList(
						knowledge.getPrimaryCategory(), mid, null);
				// 虚根
			...
			throw new WafException(
					LifeCircleErrorMessageMapper.KnowledgeCheckParamFail
							.getCode(),
					LifeCircleErrorMessageMapper.KnowledgeCheckParamFail
							.getMessage());
		}
	...
	@Override
	public void deleteInstructionalObjectives(String objectsId,
			List<String> parentNodes, String resType) {
		try {
			// 章节及挂载的课时Id集合
			...
			if (resType.equals(IndexSourceType.ChapterType.getName())) {
				for (String parentNode : parentNodes) {
					// 查找章节挂载的课时
					List<ResourceRelation> chapterRelation = resourceRelationApiService
			...
			relations.addAll(resourceRelationApiService
	 ...
	 * @since
	 */
	private Chapter getKnowledgeFromSdkWithCheck(String primaryCategory,
			String mid, String target) {
		Chapter chapter = chapterDao.getChapterFromSdk(primaryCategory, mid,
				target);
		if (chapter == null || chapter.getEnable() == null
				|| !chapter.getEnable()) {
			throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
	...
	private String getSubjectWithCheck(ResourceModel resourceModel) {
		List<ResClassificationModel> categoryList = resourceModel
				.getCategoryList();
	 ...
	 * @since
	 */
	private void updateBasicInModel(ResourceModel resourceModel,
			Education education) {
		...
		// 生命周期属性
		ResLifeCycleModel resLifeCycleModel = BeanMapperUtils.beanMapper(
				education, ResLifeCycleModel.class);
		resourceModel.setLifeCycle(resLifeCycleModel);
		// 资源的教育属性
		ResEducationalModel resEducationalModel = BeanMapperUtils.beanMapper(
				education, ResEducationalModel.class);
		...
		resourceModel.setEducationInfo(resEducationalModel);
		// 版权属性
		ResRightModel resRightModel = BeanMapperUtils.beanMapper(education,
				ResRightModel.class);
		...
		try {
			...
			if (f != null) {
				...
				Method m = resourceModel.getClass().getMethod(
						"setExtProperties", f.getType());
	 ...
	 * @since
	 */
	private Education changeModelToBean(String resourceType,
		...
		if (resourceModel.getLifeCycle() != null) {
			...
			if (resLifeCycleModel.getCreateTime() != null) {
				education.setCreateTime(new Timestamp(resLifeCycleModel
						.getCreateTime().getTime()));
			}
			if (resLifeCycleModel.getLastUpdate() != null) {
				education.setLastUpdate(new Timestamp(resLifeCycleModel
		...
		if (resourceModel.getEducationInfo() != null) {
			ResEducationalModel resEducationalModel = resourceModel
					.getEducationInfo();
			...
			education.setInteractivityLevel(resEducationalModel
			...
			if (resEducationalModel.getSemanticDensity() != null) {
				education.setSemanticDensity(resEducationalModel
						.getSemanticDensity().intValue());
			}
	 ...
	 * @since
	 */
	private boolean dealCoverages(String resourceType,
			ResourceModel resourceModel, DbName dbName) {
		...
		if (CollectionUtils.isNotEmpty(coverageModels)) {
			if (dbName.equals(DbName.DEFAULT)) {
				...
				coverageService4QuestionDB.batchCreateCoverage(coverageModels,
						true);
			}
	 ...
	 * 
	 * @see
	 * nd.esp.service.lifecycle.educommon.services.NDResourceService#update(
	 * java.lang.String,
	 * nd.esp.service.lifecycle.educommon.models.ResourceModel)
	 */
	@Override
	...
	public ResourceModel update(String resourceType,
			ResourceModel resourceModel, DbName dbName) {
		// 0、校验资源是否存在
		Education oldBean = checkResourceExist(resourceType,
				resourceModel.getIdentifier());
		...
		// 3、基本属性的处理
		dealBasicInfo(resourceType, resourceModel, OperationType.UPDATE,
				oldBean);

		// FIXME 当资源本身没有tech_info
		// 时，不需要清空,这个信息由controller传入，还是在service(commonservicehelper中配制)
		// 4、tech_info属性处理
		// 优化，当不是更新时，不必清空
		...
		// 5、categories属性处理
		// 优化，当不是更新时，不必清空
		dealCategories(resourceType, resourceModel, OperationType.UPDATE,
				dbName);
	...
	public Education checkResourceExist(String resourceType, String identifier) {
		...
		try {
			oldBean = (Education) commonServiceHelper.getRepository(
					resourceType).get(identifier);
		} catch (EspStoreException e) {
		...
		if (oldBean == null
				|| !resourceType.equals(oldBean.getPrimaryCategory())) {

			LOG.error(LifeCircleErrorMessageMapper.ChangeObjectNotExist
	...
	@Override
	public ResourceModel patch(String resourceType,
			ResourceModel resourceModel, DbName dbName) {
		// 0、校验资源是否存在
		Education oldBean = checkResourceExist(resourceType,
				resourceModel.getIdentifier());
		...
		// 3、categories属性处理
		if (resourceModel.getCategoryList() != null
				&& CollectionUtils.isNotEmpty(resourceModel.getCategoryList())) {
			dealCategoryPatch(resourceType, resourceModel);
		}

		// 4、tech_info属性处理
		if (resourceModel.getTechInfoList() != null
				&& CollectionUtils.isNotEmpty(resourceModel.getTechInfoList())) {
			dealTechInfoPatch(resourceType, resourceModel, dbName);
		}

		// 5、同步推送至报表系统
		ResourceModel rtModel = getDetail(resourceType,
				resourceModel.getIdentifier(),
				IncludesConstant.getIncludesList());
		nds.notifyReport4Resource(resourceType, rtModel, OperationType.UPDATE);
		return rtModel;
	}

	private boolean dealTechInfoPatch(String resourceType,
			ResourceModel resourceModel, DbName dbName) {
		ResourceRepository repository = commonServiceHelper
				.getTechInfoRepositoryByResType(resourceType);
		// resource id
				...
				try {
					...
					if (target != null) {
						if ("update".equals(rtim.getOperation())
								|| "add".equals(rtim.getOperation())) {
							ti = BeanMapperUtils.beanMapper(rtim,
									TechInfo.class);
							ti.setResource(uuid);
							ti.setResType(resourceType);
							ti.setRequirements(ObjectUtils.toJson(rtim
									.getRequirements()));
		...
		try {
			if (!list.isEmpty()) {
				...
				list = commonServiceHelper.getTechInfoRepositoryByResType(
						resourceType).batchAdd(list);
			}
	...
	private void dealCategoryPatch(String resourceType,
			ResourceModel resourceModel) {
		ResourceRepository repository = commonServiceHelper
				.getResourceCategoryRepositoryByResType(resourceType);
		String uuid = resourceModel.getIdentifier();
		List<ResClassificationModel> categories = resourceModel
				.getCategoryList();
		...
		if (CollectionUtils.isNotEmpty(categories)) {
			...
			try {
				categoryDatas = categoryDataRepository.getListWhereInCondition(
						"ndCode", new ArrayList<String>(ndCodeSet));
			} catch (EspStoreException e) {
			...
			if (categoryDatas.size() != ndCodeSet.size()) {
				...
				LOG.error(LifeCircleErrorMessageMapper.CheckNdCodeFail
						.getMessage() + ndCodeSet);
			...
			// 取 分类维度到shortName的映射
			Map<String, String> categoryNdCodeToShortNameMap = commonServiceHelper
					.getCategoryByData(categoryDatas);
			...
			for (CategoryData cd : categoryDatas) {
				for (ResClassificationModel resClassificationModel : resClassificationModelSet) {
					if (resClassificationModel.getTaxoncode().equals(
							cd.getNdCode())) {
						// 通过取维度数据详情，补全resource_category中间表的数据
						resClassificationModel.setShortName(cd.getShortName());
						resClassificationModel.setTaxoncodeId(cd
								.getIdentifier());
						resClassificationModel.setTaxonname(cd.getTitle());
						resClassificationModel.setCategoryCode(cd.getNdCode()
								.substring(0, 2));

						// 取维度的shortName
						if (categoryNdCodeToShortNameMap
								.get(resClassificationModel.getCategoryCode()) != null) {
							resClassificationModel
									.setCategoryName(categoryNdCodeToShortNameMap
											.get(resClassificationModel
													.getCategoryCode()));
						}

						// 转换到sdk bean
						ResourceCategory resourceCategory = BeanMapperUtils
								.beanMapper(resClassificationModel,
										ResourceCategory.class);
						resourceCategory.setResource(uuid);
						// 资源分类维度
						resourceCategory.setPrimaryCategory(resourceType);

						if (StringUtils.isEmpty(resClassificationModel
								.getOperation())) {
						...
						case "add":
							resourceCategory.setIdentifier(UUID.randomUUID()
									.toString());
						...
						case "update":
							if (resourceCategory.getIdentifier() != null) {
								try {
									ResourceCategory bean = (ResourceCategory) repository
											.get(resourceCategory
													.getIdentifier());
									if (null != bean
											&& uuid.equals(bean.getResource())) {
										resourceCategories
						...
						case "delete":
							try {
								if (resourceCategory.getIdentifier() != null) {
									repository.del(resourceCategory
											.getIdentifier());
								} else {
									ResourceCategory bean = (ResourceCategory) repository
											.getByExample(resourceCategory);
									if (null != bean) {
			...
			if (!resourceCategories.isEmpty()) {
				try {
					...
					resourceCategories = repository
							.batchAdd(resourceCategories);
	...
	private void dealBasicInfoPatch(String resourceType,
			ResourceModel resourceModel, Education oldBean) {
		@SuppressWarnings("rawtypes")
		ResourceRepository resourceRepository = commonServiceHelper
				.getRepository(resourceType);
		// 转换为数据模型
		...
		Education initEdu = BeanMapperUtils.beanMapper(
				new ResourceViewModel().getLifeCycle(), Education.class);

		Set<String> ignoreSet = Sets.newHashSet("serialVersionUID",
				"PROP_CREATETIME", "PROP_LASTUPDATE", "PROP_CATEGORYS",
				"PROP_RELATIONS", "PROP_TAGS", "PROP_KEYWORDS", "mIdentifier",
				...
				if (!ignoreSet.contains(name)) {
					...
					if (o != null && !o.equals(initValue)) {
						if (!"creator".equals(name)
								|| StringUtils.isNotEmpty((String) o)) {
							String setterName = "set"
									+ name.substring(0, 1).toUpperCase()
									+ name.substring(1);
							m = Education.class.getMethod(setterName,
									fs[i].getType());
							m.invoke(oldBean, o);
						}
					}
	 ...
	 * @since
	 */
	private boolean dealCategories(String resourceType,
			ResourceModel resourceModel, OperationType operationType,
			DbName dbName) {
		// 后期应该使用上resourceType
		// rsource uuid
			...
			if (!resourceCategories.isEmpty()) {
				try {
					...
					if (OperationType.UPDATE == operationType) {
						commonDao.deleteResourceCategoryByResource(
								resourceType, uuid, dbName);
					}

					LOG.debug("调用sdk方法：batchAdd");

					resourceCategories = commonServiceHelper
							.getResourceCategoryRepositoryByResType(
									resourceType).batchAdd(resourceCategories);
					// 返回值转换为model
					if (CollectionUtils.isNotEmpty(resourceCategories)) {
						categories = new ArrayList<ResClassificationModel>();
						for (ResourceCategory resourceCategory : resourceCategories) {
							ResClassificationModel model = BeanMapperUtils
									.beanMapper(resourceCategory,
											ResClassificationModel.class);
	 ...
	 * @since
	 */
	private boolean dealRelations(String resourceType,
			ResourceModel resourceModel, DbName dbName) {
		...
		if (CollectionUtils.isNotEmpty(relationModels)) {
			for (ResRelationModel relationModel : relationModels) {
				...
				if (lc != null) {
					...
					if (lc.getCreateTime() != null) {
						erm.setTargetCT(new BigDecimal(lc.getCreateTime()
								.getTime()));
					}
				}
			...
			Set<ResRelationModel> resRelationModelSet = new HashSet<ResRelationModel>(
					relationModels);
			if (resRelationModelSet.size() != relationModels.size()) {
				throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
						LifeCircleErrorMessageMapper.RelationDuplication
								.getCode(),
						LifeCircleErrorMessageMapper.RelationDuplication
								.getMessage());
			}
		...
		if (CollectionUtils.isNotEmpty(educationRelationModels)) {
			...
			Iterator<EducationRelationModel> it = educationRelationModels
					.iterator();
			while (it.hasNext()) {
			...
			if (CollectionUtils.isNotEmpty(questionList)) {
				educationRelationService4QuestionDB.createRelation(
						questionList, true);
			}
			if (CollectionUtils.isNotEmpty(educationRelationModels)) {
				educationRelationService.createRelation(
						educationRelationModels, true);
			}
		}
	 ...
	 * @since
	 */
	private boolean dealTechInfo(String resourceType,
			ResourceModel resourceModel, OperationType operationType,
			DbName dbName) {
		// resource id
		String uuid = resourceModel.getIdentifier();
		...
		try {
			...
			if (!list.isEmpty()) {
				...
				for (TechInfo techInfo : list) {
					// 数据回填
					ResTechInfoModel rtim = BeanMapperUtils.beanMapper(
							techInfo, ResTechInfoModel.class);
					// requirements单独处理
					List<TechnologyRequirementModel> req = ObjectUtils
							.fromJson(
									techInfo.getRequirements(),
									new TypeToken<List<TechnologyRequirementModel>>() {
									});
	...
	public ResourceViewModel createNewVersion(String resType, String uuid,
			VersionViewModel vvm, UserInfo userInfo) {
		ResourceModel rm = getDetail(resType, uuid, Arrays.asList(
				IncludesConstant.INCLUDE_LC, IncludesConstant.INCLUDE_EDU,
				IncludesConstant.INCLUDE_CR));
		// 1、判断是否可以创建版本
		if (rm.getmIdentifier() != null
				&& !rm.getIdentifier().equals(rm.getmIdentifier())) {
			throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
					"LC/CREATE_NEW_VERSION_ERROR", "已是版本分支，不允许创建新版本");
		}
		...
		if (CollectionUtils.isNotEmpty(list)) {
			for (Map<String, Object> map : list) {
				...
				if (version.equals(v)) {
					throw new LifeCircleException(
							HttpStatus.INTERNAL_SERVER_ERROR,
							"LC/CREATE_NEW_VERSION_ERROR", "已存在此版本号");
				}
			}
		}
		...
		// 5、创建资源生命周期
		LifecycleServiceV06 service = CommonServiceHelper.isQuestionDb(resType) ? lifecycleService4Qti
				: lifecycleService;
		...
		if (userInfo != null) {
			try {
				...
				List<UserCenterRoleDetails> userRoleList = userInfo
						.getUserRoles();
		...
		ResourceViewModel resourceViewModel = CommonHelper.convertViewModelOut(
				resourceModel, ResourceViewModel.class, type);
		return resourceViewModel;
	}

	@Override
	@Transactional(value = "questionTransactionManager")
	public ResourceViewModel createNewVersion4Question(String resType,
			String uuid, VersionViewModel vvm, UserInfo userInfo) {
		ResourceModel rm = getDetail(resType, uuid, Arrays.asList(
				IncludesConstant.INCLUDE_LC, IncludesConstant.INCLUDE_EDU,
				IncludesConstant.INCLUDE_CR));
		// 1、判断是否可以创建版本
	...
	@Override
	public Map<String, Map<String, Object>> versionCheck(String resType,
			String uuid) {
		ResourceModel rm = getDetail(resType, uuid,
				Arrays.asList(IncludesConstant.INCLUDE_LC));
	...
	public Map<String, Object> versionRelease(String resType, String uuid,
		...
		if (CollectionUtils.isNotEmpty(list)) {
			for (Map<String, Object> map : list) {
				...
				if (paramMap.containsKey(identifier)) {
					if (status != null
							&& !status.equals(paramMap.get(identifier))) {
						String s = updateStatusSql(resType, identifier,
								paramMap.get(identifier));
						sqls.add(s);
						ids.add(identifier);
					}
				} else {
		...
		List<Map<String, Object>> list2 = ndResourceDao.queryResourceByMid(
				resType, mid);
		Map<String, Object> returnMap = new HashMap<String, Object>();
		if (CollectionUtils.isNotEmpty(list2)) {
			for (Map<String, Object> map : list2) {
				returnMap.put((String) map.get("identifier"),
	...
	private String updateStatusSql(String resType, String uuid, String status) {
		return "update ndresource set estatus='" + status + "',last_update="
				+ System.currentTimeMillis() + " where primary_category='"
				+ resType + "' and identifier = '" + uuid + "' and enable = 1";
	}
	...
	public Map<String, ChapterStatisticsViewModel> statisticsCountsByChapters(
		...
		return ndResourceDao.statisticsCountsByChapters(resType, tmId,
				chapterIds, coverages, categories, isAll);
	}

	@Override
	public Map<String, Object> triggerTranscode(String resType, String uuid,
			boolean bStatusBackup) {
		...
		ResourceModel cm = getDetail(
				resType,
				uuid,
				IncludesConstant.getValidIncludes(IncludesConstant.INCLUDE_TI
						+ "," + IncludesConstant.INCLUDE_LC + ","
						+ IncludesConstant.INCLUDE_CG));

		String statusBackup = null;
		if (bStatusBackup
				&& !(cm.getLifeCycle().getStatus() != null && cm.getLifeCycle()
						.getStatus().contains("TRANSCOD"))) {
		...
		contributeModel.setLifecycleStatus(TransCodeUtil
				.getTransIngStatus(true));
		contributeModel.setProcess(0.0f);
		LifecycleServiceV06 service = CommonServiceHelper.isQuestionDb(resType) ? lifecycleService4Qti
				: lifecycleService;
		service.addLifecycleStep(resType, cm.getIdentifier(), contributeModel,
				false);
		resLifecycleDao.updateLifecycleStatus(resType, uuid,
				TransCodeUtil.getTransIngStatus(true));
	...
	private String dealExceptionId(List<Map<String, Object>> list) {
		List<String> existIds = new ArrayList<>();
		List<String> mids = new ArrayList<>();
		// 取出mid
		for (Map<String, Object> map : list) {
			String mid = (String) map.get("m_identifier");
			mids.add(mid);
		}
		// mid查重
		for (int i = 0; mids.size() > 0;) {
			String mid = mids.get(i);
			mids.remove(mid);
			if ((!existIds.contains(mid)) && mids.contains(mid)) {
				existIds.add(mid);
			}
		}
		// 输出重复的mid
