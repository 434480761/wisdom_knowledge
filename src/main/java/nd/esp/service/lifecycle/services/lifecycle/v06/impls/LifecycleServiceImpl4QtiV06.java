...
//        UpdatePropertyInResource(resType, entry, rtContribute.getLifeStatus(), bUpdateTime);
     ...
     */
    @Override
    public ListViewModel<ResContributeViewModel> getLifecycleSteps(String resType, String resId, String limit,String status) {
     ...
     * @param limit
     * @Method GET
     * @urlpattern {res_type}/batch/lifecycle/steps
     */
    @Override
    public Map<String, ListViewModel<ResContributeViewModel>> batchGetLifecycleSteps(String resType, Set<String> rid, String limit) {
        ...
        try {
            contributeList = contribute4QuestionRepository.getAllbySource(uuids,resType);
            contributeResult = new PageImpl<Contribute>(contributeList,pageable,(long)contributeList.size());
            for (String id : rid) {
                ListViewModel<ResContributeViewModel> list = new ListViewModel<ResContributeViewModel>();
                list.setLimit(limit);
                if (contributeResult != null) {
                    List<ResContributeViewModel> items = new ArrayList<ResContributeViewModel>();
                    for (Contribute contribute : contributeResult) {
