...
//        }
	 ...
	 * @since
	 */
	public static void resourceExistInRecycled(String resType, String resId, String type) {
        EspEntity flag = null;
        try {
            /*
             * 调用各个资源的获取详细方法,用于判断对应资源是否存在, 若不存在,则抛出异常
            ...
            if (ResourceType.RESOURCE_SOURCE.equals(type)) {
                ...
                LOG.error("源资源:" + resType + "--" + resId + "在回收站中未找到", e);
            ...
            if (ResourceType.RESOURCE_TARGET.equals(type)) {
                ...
                LOG.error("目标资源:" + resType + "--" + resId + "在回收站中未找到", e);
        ...
        // 资源不存在,抛出异常
        if (flag == null || ((flag instanceof Education) && ((Education) flag).getEnable() != null
                && ((Education) flag).getEnable() || (!((Education) flag).getEnable() 
                		&& !((Education) flag).getStatus().equals(LifecycleStatus.RECYCLED.getCode()))) 
        		|| !((Education)flag).getPrimaryCategory().equals(resType)) {
            if (ResourceType.RESOURCE_SOURCE.equals(type)) {

                LOG.error("源资源:" + resType + "--" + resId + "在回收站中未找到");

                throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
                                              LifeCircleErrorMessageMapper.SourceResourceNotFond.getCode(),
                                              "源资源:" + resType + "--" + resId + "在回收站中未找到");
            }
            if (ResourceType.RESOURCE_TARGET.equals(type)) {

                LOG.error("目标资源:" + resType + "--" + resId + "在回收站中未找到");

                throw new LifeCircleException(HttpStatus.INTERNAL_SERVER_ERROR,
                                              LifeCircleErrorMessageMapper.TargetResourceNotFond.getCode(),
                                              "目标资源:" + resType + "--" + resId + "在回收站中未找到");
            }
        }
    }
	 ...
	 * @return
	 */
	public static String EscapeSpecial(String words){
		String replaceString[]={"\\\\\\\\","\\\\\\$","\\\\\\*","\\\\\\(","\\\\\\)","\\\\\\+","\\\\","\\\\\\[","\\\\\\]","\\\\\\?","\\\\\\^","\\\\\\{","\\\\\\}","\\\\\\|"};
		String result[]={"\\\\","\\$","\\*","\\(","\\)","\\+","\\.","\\[","\\]","\\?","\\^","\\{","\\}","\\|"};
		String replaceResult=words;
		String replace=words;
		if(words!=null){
			for(int i=0;i<replaceString.length;i++){
		           replace=Pattern.compile(result[i]).matcher(replaceResult).replaceAll(replaceString[i]);
